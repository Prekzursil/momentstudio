...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "backoff_schedule_json",
            sa.Text(),
            nullable=False,
            server_default=sa.text("'[30,120,600,1800]'"),
        ),
    ...
    op.create_index(
        "ix_media_job_retry_policies_job_type",
        "media_job_retry_policies",
        ["job_type"],
        unique=True,
    )
        ...
        rows.append(
            {
                ...
                "backoff_schedule_json": json.dumps(
                    policy["schedule"], separators=(",", ":")
                ),
...
def downgrade() -> None:
    op.drop_index(
        "ix_media_job_retry_policies_updated_by_user_id",
        table_name="media_job_retry_policies",
    )
    op.drop_index(
        "ix_media_job_retry_policies_job_type", table_name="media_job_retry_policies"
    )

...
/** Golden WU admin-order-detail-cmp-apply-address-postal-suggestion -- applyAddressPostalSuggestion. */
describe('AdminOrderDetailComponent applyAddressPostalSuggestion (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyAddressPostalSuggestion(''); }).not.toThrow();
  });
});

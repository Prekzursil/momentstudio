...
/** Golden WU admin-order-detail-cmp-allowed-next-statuses -- allowedNextStatuses. */
describe('AdminOrderDetailComponent allowedNextStatuses (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).allowedNextStatuses(undefined as any, '', undefined as any); }).not.toThrow();
  });
});

  ...
  template: `
    ...
    >
      ...
      <div class="flex flex-wrap gap-3">
        ...
        >
          <span
            class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#003087] text-xs font-bold text-white"
            >P</span
          >
        ...
        >
          <span
            class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#635BFF] text-xs font-bold text-white"
            >S</span
          >
      ...
      <p class="text-xs text-slate-600 dark:text-slate-300" *ngIf="vm.paymentMethod === 'cod'">
        ...
        <a
          class="ml-1 underline text-indigo-700 hover:text-indigo-800 dark:text-indigo-300 dark:hover:text-indigo-200"
          routerLink="/contact"
        >
      ...
      <p class="text-xs text-slate-600 dark:text-slate-300" *ngIf="vm.paymentMethod === 'netopia'">
        ...
        <ng-template #netopiaDisabled>
          <span>
            ...
            <ng-container *ngIf="vm.netopiaDisabledReason">
              ({{ vm.netopiaDisabledReason }})</ng-container
            >
          </span>
        </ng-template>
        <a
      ...
      <p class="text-xs text-slate-600 dark:text-slate-300" *ngIf="vm.paymentMethod === 'paypal'">
        <span>{{
          vm.isPaymentMethodAvailable('paypal')
      ...
      <div
        class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-950/30 dark:text-slate-300"
      >
        <app-status-badge value="accepted" [labelKey]="'checkout.acceptedCards'"></app-status-badge>
        <div
          class="rounded-lg bg-white px-2 py-1 shadow-sm ring-1 ring-slate-200 dark:bg-slate-50 dark:ring-slate-200"
        >
          <img
            src="assets/payments/netopia-visa-mastercard.png"
      ...
      <div
        class="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-950/30 dark:text-slate-200"
      >
        <div
          *ngIf="vm.legalConsentsLoading"
          class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300"
        >
          <span
            class="inline-block h-3 w-3 animate-spin rounded-full border-2 border-slate-300 border-t-indigo-600 dark:border-slate-700 dark:border-t-indigo-300"
          ></span>
        ...
        <label class="flex items-start gap-2">
          <input
            type="checkbox"
            [checked]="vm.analyticsOptIn"
            (change)="vm.setAnalyticsOptIn($any($event.target).checked)"
          />
          <span>{{ 'checkout.analyticsOptInLabel' | translate }}</span>
        </label>
        <p class="text-xs text-slate-600 dark:text-slate-300">
          {{ 'checkout.analyticsOptInCopy' | translate }}
        </p>

        <p *ngIf="vm.consentError" class="text-xs text-rose-700 dark:text-rose-300">
          {{ vm.consentError }}
        </p>
      </div>
      <div
        *ngIf="vm.paymentNotReady"
        class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300"
      >
        <span
          class="inline-block h-3 w-3 animate-spin rounded-full border-2 border-slate-300 border-t-indigo-600 dark:border-slate-700 dark:border-t-indigo-300"
    ...
    <app-action-bar [stickyOnMobile]="true">
      <app-button
        [label]="
          vm.placing ? ('checkout.placingOrder' | translate) : ('checkout.placeOrder' | translate)
        "
        type="submit"
      ...
      <app-button
        variant="ghost"
        [label]="'checkout.backToCart' | translate"
        routerLink="/cart"
      ></app-button>
    </app-action-bar>
  `,

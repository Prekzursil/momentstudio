...
def test_register_and_login_flow(test_app: Dict[str, object]) -> None:
    ...
    register_payload = make_register_payload(
        email="user@example.com", username="user", name="User"
    )
    ...
    res = client.post(
        "/api/v1/auth/login",
        json={"identifier": "user@example.com", "password": "supersecret"},
    )
    ...
    # Login by username
    res = client.post(
        "/api/v1/auth/login", json={"identifier": "user", "password": "supersecret"}
    )
    assert res.status_code == 200, res.text
...
def test_register_records_legal_consents(test_app: Dict[str, object]) -> None:
    ...
    register_payload = make_register_payload(
        email="consented@example.com", username="consented", name="Consented"
    )
    res = client.post("/api/v1/auth/register", json=register_payload)
    assert res.status_code == 201, res.text
            ...
            rows = (
                (
                    await session.execute(
                        select(LegalConsent).where(LegalConsent.user_id == user_id)
                    )
                )
                .scalars()
                .all()
    ...
    assert {c.doc_key for c in consents} == {
...
def test_cooldowns_endpoint_returns_next_allowed_times(
    ...
    register_payload = make_register_payload(
        email="cooldowns@example.com", username="cooldowns", name="Cooldowns"
    )
    res = client.post("/api/v1/auth/register", json=register_payload)
    assert res.status_code == 201, res.text
...
def test_sessions_list_and_revoke_others(test_app: Dict[str, object]) -> None:
    ...
    register_payload = make_register_payload(
        email="sess@example.com", username="sess1", name="Sess"
    )
    res = client.post("/api/v1/auth/register", json=register_payload)
    assert res.status_code == 201, res.text
    ...
    revoke = client.post(
        ...
        headers=auth_headers(access_two),
...
def test_two_factor_setup_and_login_flow(test_app: Dict[str, object]) -> None:
    ...
    login = client.post(
        "/api/v1/auth/login",
        json={"identifier": "2fa@example.com", "password": "supersecret"},
    )
    ...
    login3 = client.post(
        "/api/v1/auth/login/2fa",
        json={"two_factor_token": token3, "code": recovery_code},
    )
...
def test_passkeys_register_list_and_login_flow(
    ...
    auth_opts = client.post(
        "/api/v1/auth/passkeys/login/options",
        json={"identifier": "passkeys", "remember": True},
    )
    ...
    login = client.post(
        ...
        json={
            ...
            "credential": {"rawId": raw_id, "id": raw_id},
        },
    )
    assert login.status_code == 200, login.text
    body = login.json()
...
def test_secondary_emails_flow(
    monkeypatch: pytest.MonkeyPatch, test_app: Dict[str, object]
) -> None:
    client: TestClient = test_app["client"]  # type: ignore[assignment]
    ...
    confirm = client.post(
        "/api/v1/auth/me/emails/verify/confirm", json={"token": sent["alt@example.com"]}
    )
    assert confirm.status_code == 200, confirm.text
    assert confirm.json()["verified"] is True

    login_secondary = client.post(
        "/api/v1/auth/login",
        json={"identifier": "alt@example.com", "password": "supersecret"},
    )
    ...
    old_secondary = next(
        (e for e in body["secondary_emails"] if e["email"] == "user@example.com"), None
    )
...
def test_primary_email_verification_strips_whitespace(
    monkeypatch: pytest.MonkeyPatch, test_app: Dict[str, object]
) -> None:
    client: TestClient = test_app["client"]  # type: ignore[assignment]
...
def test_register_rejects_invalid_phone(test_app: Dict[str, object]) -> None:
    ...
    res = client.post(
        "/api/v1/auth/register",
        json=make_register_payload(
            email="badphone@example.com", username="badphone", phone="0723204204"
        ),
    )
...
def test_register_rejects_future_date_of_birth(test_app: Dict[str, object]) -> None:
    ...
    res = client.post(
        "/api/v1/auth/register",
        json=make_register_payload(
            email="baddob@example.com", username="baddob", date_of_birth="2999-01-01"
        ),
    )
    assert res.status_code == 422, res.text
...
def test_invalid_login_and_refresh(test_app: Dict[str, object]) -> None:
    ...
    res = client.post(
        "/api/v1/auth/login",
        json={"email": "nobody@example.com", "password": "invalidpw"},
    )
...
def test_silent_refresh_probe_returns_204_for_missing_or_invalid_token(
    ...
    non_silent_invalid = client.post(
...
def test_admin_guard(test_app: Dict[str, object]) -> None:
    ...
    res = client.get(
        "/api/v1/auth/admin/ping", headers={"Authorization": f"Bearer {admin_access}"}
    )
...
def test_password_reset_flow(
    ...
    login = client.post(
        "/api/v1/auth/login",
        json={"identifier": "reset@example.com", "password": "newsecret"},
    )
...
def test_update_profile_me(test_app: Dict[str, object]) -> None:
    ...
    res = client.post(
        "/api/v1/auth/register",
        json=make_register_payload(
            email="me@example.com", username="me1", password="supersecret", name="Old"
        ),
    )
    assert res.status_code == 201, res.text
    ...
    async def rewind_display_name_cooldown() -> None:
        async with SessionLocal() as session:
            user = (
                await session.execute(
                    select(User).where(User.email == "me@example.com")
                )
            ).scalar_one()
            history = (
                (
...
def test_change_password_persists_and_updates_login(
    ...
    res = client.post(
        "/api/v1/auth/register",
        json=make_register_payload(
            email="pw@example.com", username="pw1", password="oldsecret", name="PW"
        ),
    )
    assert res.status_code == 201, res.text
    token = res.json()["tokens"]["access_token"]
    ...
    old_login = client.post(
        "/api/v1/auth/login",
        json={"identifier": "pw@example.com", "password": "oldsecret"},
    )
    assert old_login.status_code == 401

    new_login = client.post(
        "/api/v1/auth/login",
        json={"identifier": "pw@example.com", "password": "newsecret"},
    )
...
def test_alias_history_and_display_name_tags(test_app: Dict[str, object]) -> None:
    ...
    async def rewind_username_cooldown() -> None:
        async with SessionLocal() as session:
            user = (
                await session.execute(
                    select(User).where(User.email == "ana1@example.com")
                )
            ).scalar_one()
            history = (
                (
...
def test_display_name_history_reuses_name_tag_on_revert(
    ...
    async def rewind_display_name_cooldown() -> None:
        async with SessionLocal() as session:
            user = (
                await session.execute(
                    select(User).where(User.email == "tagreuse1@example.com")
                )
            ).scalar_one()
            history = (
                (
    ...
    assert aliases.status_code == 200, aliases.text
    display_names = [
        (h["name"], h["name_tag"]) for h in aliases.json()["display_names"]
    ]
    assert display_names[0] == ("Ana", 0)
    assert ("Maria", 0) in display_names

    bad = client.patch(
        "/api/v1/auth/me", json={"name": "   "}, headers=auth_headers(token)
    )

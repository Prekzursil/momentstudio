...
/** Golden WU checkout-cmp-fn-handle-checkout-start-response -- handleCheckoutStartResponse. */
describe('CheckoutComponent handleCheckoutStartResponse (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).handleCheckoutStartResponse(undefined as any); }).not.toThrow();
  });
});

    ...
    for product in products:
        ...
        if not _is_valid_currency(product.currency or ""):
            add(
                ...
                slug,
                "error",
                f"Invalid currency '{product.currency}' (expected ISO 4217)",
            )
        ...
        if price <= 0:
            add(
                ...
                slug,
                "warning",
                "Offer price is 0 (rich results may be affected)",
            )
        if (
            not (product.short_description or "").strip()
            and not (product.long_description or "").strip()
        ):
            add(
                "product",
                slug,
                "warning",
                "Missing product description (short or long)",
            )
        if not getattr(product, "images", []) or len(product.images) == 0:
            add(
                ...
                slug,
                "warning",
                "Missing product images (recommended for rich results)",
            )
        else:
            for img in product.images:
                ...
                if url and not _is_absolute_url(url):
                    add(
                        ...
                        slug,
                        "warning",
                        "Image URL should be absolute (include https://)",
                    )
                    break
        ...
        if not _is_absolute_url(canonical):
            add(
                ...
                slug,
                "error",
                "Invalid frontend origin; cannot build absolute product URLs",
            )
    ...
    for page in pages:
        ...
        if not slug:
            add(
                ...
                key or "<missing>",
                "error",
                "Invalid page key (expected page.<slug>)",
            )
            continue
        meta = page.meta or {}
        ...
        if not _is_absolute_url(url):
            add(
                ...
                key,
                "error",
                "Invalid frontend origin; cannot build absolute page URLs",
            )

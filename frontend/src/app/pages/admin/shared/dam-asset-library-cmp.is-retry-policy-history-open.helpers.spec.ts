...
/** Golden WU dam-asset-library-cmp-is-retry-policy-history-open -- isRetryPolicyHistoryOpen. */
describe('DamAssetLibraryComponent isRetryPolicyHistoryOpen (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).isRetryPolicyHistoryOpen(undefined as any); }).not.toThrow();
  });
});

...
featured_collection_products = Table(
    ...
    Column(
        "collection_id",
        UUID(as_uuid=True),
        ForeignKey("featured_collections.id"),
        primary_key=True,
    ),
    Column(
        "product_id", UUID(as_uuid=True), ForeignKey("products.id"), primary_key=True
...
class Product(Base):
    ...
    sale_auto_publish: Mapped[bool] = mapped_column(
    ...
    restock_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    shipping_class: Mapped[ShippingClass] = mapped_column(
        Enum(ShippingClass, native_enum=False),
        nullable=False,
        default=ShippingClass.standard,
    )
    shipping_allow_locker: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, server_default="true"
    )
    shipping_disallowed_couriers: Mapped[list[str]] = mapped_column(
    ...
    publish_scheduled_for: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, index=True
    )
    unpublish_scheduled_for: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, index=True
    )
...
class ProductBadge(Base):
    ...
    __table_args__ = (
        UniqueConstraint("product_id", "badge", name="uq_product_badges_unique"),
    )
...
class ProductRelationship(Base):
    ...
    relationship_type: Mapped[ProductRelationshipType] = mapped_column(
        Enum(
            ProductRelationshipType, name="productrelationshiptype", native_enum=False
        ),
        nullable=False,
        index=True,
    )
    ...
    related_product: Mapped["Product"] = relationship(
        "Product", foreign_keys=[related_product_id], lazy="joined"
    )
...
class ProductImageTranslation(Base):
    ...
    image: Mapped["ProductImage"] = relationship(
        "ProductImage", back_populates="translations"
    )
...
class ProductVariant(Base):
    ...
    name: Mapped[str] = mapped_column(String(120), nullable=False)
    additional_price_delta: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=0
    )
    stock_quantity: Mapped[int] = mapped_column(nullable=False, default=0)
    created_at: Mapped[datetime] = mapped_column(
...
class StockAdjustment(Base):
    ...
    reason: Mapped[StockAdjustmentReason] = mapped_column(
        Enum(StockAdjustmentReason, native_enum=False), nullable=False
    )
...
class RestockNote(Base):
    ...
    __table_args__ = (
        UniqueConstraint("target_key", name="uq_restock_notes_target_key"),
    )
...
class FeaturedCollection(Base):
    ...
    products: Mapped[list[Product]] = relationship(
        "Product",
        secondary=featured_collection_products,
        back_populates="featured_collections",
        lazy="selectin",
    )

...
/** Golden WU http-error-bus-svc-emit -- emit. */
describe('HttpErrorBusService emit (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(HttpErrorBusService.prototype) as HttpErrorBusService;
    ...
    expect(() => { (cmp as any).emit({} as any); }).not.toThrow();
  });
});

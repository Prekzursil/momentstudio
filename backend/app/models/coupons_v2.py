...
class Promotion(Base):
    ...
    allow_on_sale_items: Mapped[bool] = mapped_column(
...
class Coupon(Base):
    ...
    per_customer_max_redemptions: Mapped[int | None] = mapped_column(
...
class CouponAssignment(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "coupon_id", "user_id", name="uq_coupon_assignments_coupon_user"
        ),
    )
...
class CouponReservation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("order_id", name="uq_coupon_reservations_order"),
    )
    ...
    discount_ron: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=0
    )
    shipping_discount_ron: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=0
    )
...
class CouponBulkJob(Base):
    ...
    require_marketing_opt_in: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    ...
    restored: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    already_active: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    revoked: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    already_revoked: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )
    not_assigned: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, server_default="0"
    )

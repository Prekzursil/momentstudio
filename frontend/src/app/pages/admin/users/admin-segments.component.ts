  ...
  template: `
    <div class="grid gap-6">
      ...
      <app-admin-page-header
        [titleKey]="'adminUi.segments.title'"
        [hintKey]="'adminUi.segments.hint'"
      ></app-admin-page-header>
      ...
      <section class="grid gap-4 lg:grid-cols-2 items-start">
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="grid gap-1">
              <h2 class="text-base font-semibold text-slate-900 dark:text-slate-50">
                {{ 'adminUi.segments.repeatBuyers.title' | translate }}
              </h2>
                ...
                <thead class="bg-slate-50 text-slate-700 dark:bg-slate-800/70 dark:text-slate-200">
                  <tr>
                    <th class="text-left font-semibold px-3 py-2">
                      {{ 'adminUi.segments.table.user' | translate }}
                    </th>
                    <th class="text-right font-semibold px-3 py-2">
                      {{ 'adminUi.segments.table.orders' | translate }}
                    </th>
                    <th class="text-right font-semibold px-3 py-2">
                      {{ 'adminUi.segments.table.total' | translate }}
                    </th>
                    <th class="text-right font-semibold px-3 py-2">
                      {{ 'adminUi.segments.table.aov' | translate }}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    *ngFor="let row of repeatItems(); trackBy: trackRow"
        ...
        >
          <div class="flex items-start justify-between gap-3">
            <div class="grid gap-1">
              <h2 class="text-base font-semibold text-slate-900 dark:text-slate-50">
                {{ 'adminUi.segments.highAov.title' | translate }}
              </h2>
          ...
          <ng-template #aovTpl>
            ...
            >
              <table class="min-w-[820px] w-full text-sm">
                ...
                <tbody>
                  <tr
                    *ngFor="let row of aovItems(); trackBy: trackRow"
                    class="border-t border-slate-200 dark:border-slate-800"
                  >
                    <td class="px-3 py-2">
                      <div class="grid gap-0.5">
  ...
  private loadRepeat(): void {
    ...
    this.usersApi
      ...
      .subscribe({
        ...
        error: () => {
          this.repeatError.set(
            this.translate.instant('adminUi.segments.errors.repeatLoad') as string,
          );
          this.repeatLoading.set(false);
        },
      });

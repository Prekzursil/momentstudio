...
/** Golden WU admin-cmp-category-wizard-next -- categoryWizardNext. */
describe('AdminComponent categoryWizardNext (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).categoryWizardNext(); }).not.toThrow();
  });
});

...
/** Golden WU admin-coupons-cmp-stacking-preview-product-discount -- stackingPreviewProductDiscount. */
describe('AdminCouponsComponent stackingPreviewProductDiscount (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).stackingPreviewProductDiscount(false); }).not.toThrow();
  });
});

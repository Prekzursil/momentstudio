...
/** Golden WU form-messages-svc-get-error -- getError. */
describe('FormMessagesService getError (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(FormMessagesService.prototype) as FormMessagesService;
    ...
    expect(() => { (cmp as any).getError(undefined as any); }).not.toThrow();
  });
});

...
/** Golden WU dam-asset-library-cmp-retry-policy-history-error -- retryPolicyHistoryError. */
describe('DamAssetLibraryComponent retryPolicyHistoryError (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryPolicyHistoryError(undefined as any); }).not.toThrow();
  });
});

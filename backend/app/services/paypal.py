...
async def _fx_per_ron(
    currency_code: str, *, fx_eur_per_ron: float | None, fx_usd_per_ron: float | None
) -> Decimal:
    ...
    return Decimal(
        str(fetched.usd_per_ron if currency_code == "USD" else fetched.eur_per_ron)
    )


def _convert_ron(value_ron: Decimal, fx_per_ron: Decimal) -> Decimal:
    return (Decimal(value_ron) * fx_per_ron).quantize(
...
def _effective_client_id() -> str:
    if _paypal_env() == "live":
        return (
            settings.paypal_client_id_live or settings.paypal_client_id or ""
        ).strip()
    return (
        settings.paypal_client_id_sandbox or settings.paypal_client_id or ""
    ).strip()
...
def _effective_client_secret() -> str:
    if _paypal_env() == "live":
        return (
            settings.paypal_client_secret_live or settings.paypal_client_secret or ""
        ).strip()
    return (
        settings.paypal_client_secret_sandbox or settings.paypal_client_secret or ""
    ).strip()
...
def _effective_webhook_id() -> str:
    if _paypal_env() == "live":
        return (
            settings.paypal_webhook_id_live or settings.paypal_webhook_id or ""
        ).strip()
    return (
        settings.paypal_webhook_id_sandbox or settings.paypal_webhook_id or ""
    ).strip()
...
def _cache_bucket() -> dict[str, object]:
    return _token_cache.setdefault(
        _paypal_env(), {"access_token": None, "expires_at": None}
    )
...
async def _get_access_token() -> str:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="PayPal token request failed",
        ) from exc
    ...
    if not isinstance(access_token, str) or not access_token:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY, detail="PayPal token missing"
        )

    expiry = now + timedelta(
        seconds=int(expires_in) if isinstance(expires_in, (int, float)) else 300
    )
...
async def create_order_itemized(
    *,
    ...
    fx_per_ron = await _fx_per_ron(
        currency, fx_eur_per_ron=fx_eur_per_ron, fx_usd_per_ron=fx_usd_per_ron
    )
    ...
    shipping_converted = (
        _convert_ron(Decimal(shipping_ron), fx_per_ron)
        if shipping_ron is not None
        else None
    )
    fee_converted = (
        _convert_ron(Decimal(fee_ron), fx_per_ron) if fee_ron is not None else None
    )
    tax_converted = (
        _convert_ron(Decimal(tax_ron), fx_per_ron) if tax_ron is not None else None
    )
    discount_converted = (
    ...
    if total_converted <= 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid PayPal order total"
        )

    amount: dict[str, Any] = {
        "currency_code": currency,
        "value": _format_amount(total_converted),
    }
    if (
    ...
    if not isinstance(paypal_order_id, str) or not paypal_order_id or not approval_url:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="PayPal approval link missing",
        )
...
def _sanitize_paypal_id(paypal_id: str) -> str:
    ...
    if not isinstance(paypal_id, str):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid PayPal order id"
        )
...
async def capture_order(*, paypal_order_id: str) -> str:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY, detail="PayPal capture failed"
        ) from exc
...
async def refund_capture(
    *,
    ...
    if amount_ron is not None:
        ...
        payload = {
            "amount": {
                "value": _format_amount(amount_converted),
                "currency_code": currency,
            }
        }
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY, detail="PayPal refund failed"
        ) from exc
...
async def verify_webhook_signature(
    *, headers: dict[str, str], event: dict[str, Any]
) -> bool:
    webhook_id = _effective_webhook_id()
    if not webhook_id:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="PayPal webhook id not configured",
        )

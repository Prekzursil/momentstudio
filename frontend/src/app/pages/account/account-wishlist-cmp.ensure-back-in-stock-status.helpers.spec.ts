...
/** Golden WU account-wishlist-cmp-ensure-back-in-stock-status -- ensureBackInStockStatus. */
describe('AccountWishlistComponent ensureBackInStockStatus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).ensureBackInStockStatus(undefined as any); }).not.toThrow();
  });
});

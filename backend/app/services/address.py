...
def _validate_address_fields(country: str, postal_code: str) -> tuple[str, str]:
    ...
    if not postal_code or not postal_code.strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Postal code is required"
        )
    ...
    if pattern:
        if not re.match(pattern, postal_code):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid postal code for country",
            )
    else:
...
async def create_address(
    session: AsyncSession, user_id, payload: AddressCreate
) -> Address:
    country, postal_code = _validate_address_fields(
        payload.country, payload.postal_code
    )
    ...
    if payload.is_default_shipping or payload.is_default_billing:
        await _clear_defaults(
            session, user_id, payload.is_default_shipping, payload.is_default_billing
        )
    session.add(address)
...
async def update_address(
    session: AsyncSession, address: Address, payload: AddressUpdate
) -> Address:
    ...
    if payload.is_default_shipping or payload.is_default_billing:
        await _clear_defaults(
            ...
            (
                ...
                if payload.is_default_billing is not None
                else False
            ),
...
async def _clear_defaults(
    session: AsyncSession, user_id, shipping: bool, billing: bool, exclude_id=None
) -> None:

...
def upgrade() -> None:
    ...
    op.add_column(
        "orders",
        sa.Column("reference_code", sa.String(length=20), nullable=True, unique=True),
    )
    op.add_column(
        "orders",
        sa.Column(
    ...
    op.add_column(
        "orders", sa.Column("tracking_number", sa.String(length=50), nullable=True)
    )
    op.add_column(
        "orders",
    ...
    if "status" in {
        col["name"] for col in sa.inspect(op.get_bind()).get_columns("orders")
    }:

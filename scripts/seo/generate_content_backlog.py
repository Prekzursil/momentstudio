...
def _classify_issue_type(rule_id: str) -> tuple[str, str]:
    ...
    if "missing_h1" in rid:
        return (
            "heading",
            "Add a unique route H1 and align above-the-fold copy with primary intent.",
        )
    if "missing_canonical" in rid:
        return (
            "canonical",
            "Set canonical URL and language alternates consistent with sitemap policy.",
        )
    ...
    if "missing_description" in rid:
        return (
            "description",
            "Add concise meta description with intent + differentiator.",
        )
    if "no_meaningful_text" in rid:
        return (
            "thin_content",
            "Add a meaningful introductory paragraph and internal links.",
        )
    if "duplicate_title" in rid:
        return (
            "duplicate_title",
            "Make title unique by route context (entity + intent).",
        )
    if "duplicate_description" in rid:
        return (
            "duplicate_description",
            "Make description unique with route-specific summary.",
        )
    if "broken_link" in rid:
        return (
            "broken_link",
            "Fix or remove broken links; replace with canonical destination.",
        )
    if "low_internal_links" in rid:
        return (
            "internal_links",
            "Add contextual internal links to related indexable routes.",
        )
    if "canonical_policy_mismatch" in rid:
        return (
            "canonical_policy",
            "Align canonical policy with sitemap language policy.",
        )
...
def build_backlog(findings: list[dict[str, Any]]) -> list[BacklogItem]:
    ...
    for finding in findings:
        ...
        labels = [
            str(label).strip().lower() for label in list(finding.get("labels") or [])
        ]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate prioritized SEO content backlog from deterministic findings."
    )
    parser.add_argument(

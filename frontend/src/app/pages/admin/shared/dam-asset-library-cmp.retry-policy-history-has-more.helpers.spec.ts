...
/** Golden WU dam-asset-library-cmp-retry-policy-history-has-more -- retryPolicyHistoryHasMore. */
describe('DamAssetLibraryComponent retryPolicyHistoryHasMore (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryPolicyHistoryHasMore(undefined as any); }).not.toThrow();
  });
});

...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_user_secondary_emails_user_id", "user_secondary_emails", ["user_id"]
    )
    op.create_index(
        "ix_user_secondary_emails_email", "user_secondary_emails", ["email"]
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "token", name="uq_secondary_email_verification_tokens_token"
        ),
    )
    op.create_index(
...
def downgrade() -> None:
    op.drop_index(
        "ix_secondary_email_verification_tokens_created_at",
        table_name="secondary_email_verification_tokens",
    )
    op.drop_index(
        "ix_secondary_email_verification_tokens_secondary_email_id",
        table_name="secondary_email_verification_tokens",
    ...
    op.drop_index(
        "ix_user_secondary_emails_user_id", table_name="user_secondary_emails"
    )

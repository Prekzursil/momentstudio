...
/** Golden WU account-state-secondary-email-resend-remaining-seconds -- secondaryEmailResendRemainingSeconds. */
describe('AccountState secondaryEmailResendRemainingSeconds (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).secondaryEmailResendRemainingSeconds(''); }).not.toThrow();
  });
});

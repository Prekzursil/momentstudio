...
@Component({
  ...
  template: `
    <div class="grid gap-4">
      ...
      >
        <div>
          <p class="font-semibold text-amber-900 dark:text-amber-100">
            {{ 'page.blocks.productGrid.errorTitle' | translate }}
          </p>
          <p class="text-sm text-amber-800 dark:text-amber-200">
            {{ 'page.blocks.productGrid.errorCopy' | translate }}
          </p>
        </div>
      ...
      <div
        *ngIf="!loading() && !error() && products().length"
...
export class CmsProductGridBlockComponent implements OnChanges, OnDestroy {
  ...
  load(): void {
    ...
    const limit = Math.max(
      1,
      Math.min(24, Number.isFinite(desiredLimit) ? Math.round(desiredLimit) : 6),
    );
    ...
    const req =
      source === 'category' && categorySlug
        ? this.catalog
            .listProducts({ category_slug: categorySlug, limit })
        ...
        : source === 'collection' && collectionSlug
          ...
          : source === 'products' && manualSlugs.length
            ? forkJoin(
                manualSlugs
                  .slice(0, limit)
                  .map((slug) => this.catalog.getProduct(slug).pipe(catchError(() => of(null)))),

...
def create_access_token(subject: str, jti: str | None = None) -> str:
    return _create_token_with_claims(
        subject, "access", timedelta(minutes=settings.access_token_exp_minutes), jti=jti
    )
...
def create_impersonation_access_token(
    ...
    *,
    ...
    minutes = int(
        ...
        else settings.admin_impersonation_exp_minutes
    )
...
def create_refresh_token(
    subject: str, jti: str, expires_at: datetime | None = None
) -> str:
    delta = (
...
def create_admin_ip_bypass_token(
    subject: str, *, expires_minutes: int | None = None
) -> str:
    minutes = int(
        expires_minutes
        if expires_minutes is not None
        else settings.admin_ip_bypass_cookie_minutes
    )
    minutes = max(1, minutes)
    return _create_token_with_claims(
        subject, "admin_ip_bypass", timedelta(minutes=minutes)
    )
...
def create_google_completion_token(subject: str) -> str:
    return _create_token_with_claims(
        subject,
        "google_completion",
        timedelta(minutes=settings.google_completion_token_exp_minutes),
    )
...
def create_receipt_token(
    *, order_id: str, expires_at: datetime, token_version: int = 0
) -> str:
    to_encode = {
        "type": "receipt",
...
def decode_receipt_token(token: str) -> tuple[str, int] | None:
    ...
    ver = (
        int(ver_raw)
        if isinstance(ver_raw, (int, float, str)) and str(ver_raw).strip()
        else 0
    )

...
/** Golden WU admin-products-cmp-category-manager-selected-category -- categoryManagerSelectedCategory. */
describe('AdminProductsComponent categoryManagerSelectedCategory (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).categoryManagerSelectedCategory(); }).not.toThrow();
  });
});

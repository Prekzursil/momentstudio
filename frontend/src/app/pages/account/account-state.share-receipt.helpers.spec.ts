...
/** Golden WU account-state-share-receipt -- shareReceipt. */
describe('AccountState shareReceipt (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).shareReceipt(undefined as any); }).not.toThrow();
  });
});

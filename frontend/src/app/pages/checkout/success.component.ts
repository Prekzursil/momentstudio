...
@Component({
  ...
  template: `
    <app-container classes="py-10 grid gap-6">
      ...
      >
        <p class="text-sm uppercase tracking-[0.3em] font-semibold">
          {{ 'checkout.successTitle' | translate }}
        </p>
        <h1 class="text-2xl font-semibold text-emerald-900 dark:text-emerald-100">
          {{ 'checkout.successHeadline' | translate }}
        </h1>
        <p class="text-sm text-emerald-800 dark:text-emerald-200">
          {{ 'checkout.successCopy' | translate }}
        </p>
        <div class="flex flex-wrap gap-3">
          <app-button
            routerLink="/shop"
            [label]="'checkout.successContinue' | translate"
          ></app-button>
          <app-button
            routerLink="/account"
            variant="ghost"
            [label]="'checkout.successViewOrders' | translate"
          ></app-button>
        </div>
      </div>
        ...
        <div class="grid gap-2 text-sm text-slate-700 dark:text-slate-200">
          <div *ngFor="let item of summary.items">
            <div class="flex justify-between gap-4">
              <a class="font-medium hover:underline" [routerLink]="['/products', item.slug]">{{
                item.name
              }}</a>
              <span>{{ item.unit_price * item.quantity | localizedCurrency: item.currency }}</span>
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-400">
              {{ item.quantity }} × {{ item.unit_price | localizedCurrency: item.currency }}
            </p>
          </div>
        </div>

        <div
          class="grid gap-1 text-sm text-slate-700 dark:text-slate-200 border-t border-slate-200 pt-3 dark:border-slate-800"
        >
          ...
          <div class="flex items-center justify-between" *ngIf="summary.totals.discount > 0">
            ...
            <span class="text-emerald-700 dark:text-emerald-300">{{
              -summary.totals.discount | localizedCurrency: summary.totals.currency
            }}</span>
          </div>
          <div
            class="flex items-center justify-between text-base font-semibold text-slate-900 pt-1 dark:text-slate-50"
          >
...
export class SuccessComponent {
  crumbs = [{ label: 'nav.home', url: '/' }, { label: 'checkout.successTitle' }];

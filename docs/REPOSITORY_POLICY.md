...
- `Chromatic Playwright` runs visual archive generation for the storefront routes on pull requests and `main`.
...
- If `CHROMATIC_PROJECT_TOKEN` is missing, Chromatic workflows fail for maintained branches and only skip in restricted Dependabot contexts.

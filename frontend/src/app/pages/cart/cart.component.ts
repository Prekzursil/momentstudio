    ...
    <app-container classes="py-10 grid gap-6">
      <app-page-header [crumbs]="crumbs" [titleKey]="'cart.title'">
        <div pageHeaderActions class="flex items-center gap-3">
          <span class="text-sm text-slate-600 dark:text-slate-300">{{
            'cart.items' | translate: { count: items().length }
          }}</span>
          <span *ngIf="syncing()" class="text-xs text-slate-500 dark:text-slate-400">{{
      ...
      <div class="grid lg:grid-cols-[2fr_1fr] gap-6 items-start">
        <section class="grid gap-4">
          ...
          <div
            *ngIf="!syncing() && !items().length"
            class="border border-dashed border-slate-200 rounded-2xl p-10 text-center grid gap-3 dark:border-slate-800"
          >
            <p class="text-lg font-semibold text-slate-900 dark:text-slate-50">
              {{ 'cart.emptyTitle' | translate }}
            </p>
              ...
              <div class="flex-1 grid gap-2">
                <div class="flex items-start justify-between">
                  <div>
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                      <ng-container *ngIf="isLowStock(item); else cartInStock">{{
                        'cart.onlyLeft' | translate: { count: item.stock }
                      }}</ng-container>
                      <ng-template #cartInStock>{{
                ...
                <div class="flex items-center gap-3 text-sm">
                  ...
                  <span class="text-slate-600 dark:text-slate-300">
                    {{ item.price | localizedCurrency: item.currency }}
                    {{ 'cart.each' | translate }}
                  </span>
                ...
                <p *ngIf="isMaxQuantity(item)" class="text-xs text-amber-700 dark:text-amber-300">
                  {{ 'cart.maxQtyReached' | translate }}
                </p>
                <p *ngIf="itemErrors[item.id]" class="text-sm text-amber-700 dark:text-amber-300">
                  {{ itemErrors[item.id] | translate }}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div
            *ngIf="items().length"
            class="grid gap-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-700 dark:bg-slate-800/40 dark:text-slate-200"
          >
          ...
          <div
            *ngIf="items().length && freeShippingThreshold() !== null"
            class="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200"
          >
            <p class="text-sm font-semibold text-slate-900 dark:text-slate-50">
              {{ 'cart.freeShippingTitle' | translate }}
            </p>
            <p
              *ngIf="freeShippingAppliedByCoupon()"
            ...
            <ng-container *ngIf="!freeShippingAppliedByCoupon()">
              <p
                *ngIf="freeShippingRemaining() !== null && freeShippingRemaining() > 0"
                class="text-sm"
              >
                {{
                  'cart.freeShippingRemaining'
                    | translate: { amount: (freeShippingRemaining() | localizedCurrency: currency) }
                }}
              </p>
              <p
                *ngIf="freeShippingRemaining() !== null && freeShippingRemaining() <= 0"
              ...
              >
                <p class="text-xs font-semibold text-slate-500 dark:text-slate-300">
                  {{ 'cart.suggestedAddOns' | translate }}
                </p>
                <a
          ...
          <div class="grid gap-2">
            ...
            <p
              *ngIf="promoMessage"
              class="text-xs"
              [class.text-emerald-700]="promoStatus === 'success'"
              [class.text-amber-700]="promoStatus === 'warn'"
              [class.dark:text-emerald-300]="promoStatus === 'success'"
              [class.dark:text-amber-300]="promoStatus === 'warn'"
            >
              {{ promoMessage }}
            </p>
          </div>
      ...
      <section *ngIf="savedForLater.length" class="grid gap-4">
        <div class="flex items-center justify-between gap-3">
          <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-50">
            {{ 'cart.savedForLaterTitle' | translate }}
          </h2>
        </div>
        <div class="grid gap-3">
          <div
            ...
            <div class="flex-1 grid gap-2">
              <div class="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p class="text-sm text-slate-600 dark:text-slate-300">
                    {{ 'cart.qty' | translate }}: {{ saved.quantity }} ·
                    {{ saved.price | localizedCurrency: saved.currency }}
                  </p>
                </div>
                <div class="flex items-center gap-3">
                  <button
      ...
      <section *ngIf="items().length" class="grid gap-4">
        <div class="flex items-center justify-between gap-3">
          <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-50">
            {{ 'cart.recommendationsTitle' | translate }}
          </h2>
          <a
            routerLink="/shop"
        ...
        <p *ngIf="recommendationsError" class="text-sm text-slate-600 dark:text-slate-300">
          {{ recommendationsError }}
        </p>

        <div
          *ngIf="!recommendationsLoading && recommendations.length"
...
export class CartComponent implements OnInit {
  crumbs = [{ label: 'nav.home', url: '/' }, { label: 'nav.cart' }];
  skeletonRows = [0, 1, 2];
  ...
  deliveryEstimate(): { min: number; max: number } | null {
    const est: Record<
      LockerProvider,
      Record<CheckoutDeliveryType, { min: number; max: number }>
    > = {
  ...
  saveForLater(item: CartItem): void {
    ...
    this.cart.remove(item.id, {
      ...
      onError: () => {
        delete this.savingForLater[item.id];
        this.toast.error(
          this.translate.instant('cart.saveForLater'),
          this.translate.instant('cart.saveForLaterFailed'),
        );
      },
    });
  }
    ...
    this.cartApi
      ...
      .subscribe({
        ...
        error: () => {
          delete this.restoringSaved[key];
          this.toast.error(
            this.translate.instant('cart.moveToCart'),
            this.translate.instant('cart.moveToCartFailed'),
          );
        },
      });
  }
  ...
  private loadSavedForLater(): SavedForLaterItem[] {
    ...
    try {
      ...
      return parsed
        ...
        .filter(
          (entry) => entry.product_id && entry.slug && entry.name && Number.isFinite(entry.price),
        );
    } catch {
      return [];
    }

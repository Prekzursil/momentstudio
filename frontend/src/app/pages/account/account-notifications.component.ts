      ...
      <ng-template #notificationsBody>
        <div class="flex items-start justify-between gap-3">
          <div class="grid gap-1">
            ...
            <p
              *ngIf="account.notificationLastUpdated"
              class="text-xs text-slate-500 dark:text-slate-400"
            >
              {{
                'account.notifications.lastUpdated'
                  | translate: { date: account.formatTimestamp(account.notificationLastUpdated) }
              }}
            </p>
          </div>
        ...
        <div class="grid gap-4 text-sm text-slate-700 dark:text-slate-200">
          <div
            class="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-200 grid gap-1"
          >
          ...
          <div class="grid gap-2">
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              >
                {{
                  preview === 'reply'
                    ? ('account.notifications.hidePreview' | translate)
                    : ('account.notifications.showPreview' | translate)
                }}
              </button>
            </div>
            <div
            ...
            >
              ...
              <p class="whitespace-pre-wrap">
                {{ 'account.notifications.previewReply' | translate }}
              </p>
            </div>
          </div>
          ...
          <div class="grid gap-2" *ngIf="account.isAdmin()">
            ...
            >
              ...
              <p class="whitespace-pre-wrap">
                {{ 'account.notifications.previewAdmin' | translate }}
              </p>
            </div>
          </div>
          ...
          <div class="grid gap-2">
            ...
            >
              ...
              <p class="whitespace-pre-wrap">
                {{ 'account.notifications.previewMarketing' | translate }}
              </p>
            </div>
          </div>

          <span
            *ngIf="account.notificationsMessage"
            class="text-xs text-emerald-700 dark:text-emerald-300"
            >{{ account.notificationsMessage | translate }}</span
          >
          <span
            *ngIf="account.notificationsError"
            class="text-xs text-rose-700 dark:text-rose-300"
            >{{ account.notificationsError | translate }}</span
          >
        </div>
      </ng-template>
    </section>

...
/** Golden WU admin-orders-cmp-download-selected-shipping-labels-zip -- downloadSelectedShippingLabelsZip. */
describe('AdminOrdersComponent downloadSelectedShippingLabelsZip (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadSelectedShippingLabelsZip(); }).not.toThrow();
  });
});

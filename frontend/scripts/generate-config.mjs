...
const fxEurPerRon = Number(process.env.FX_EUR_PER_RON ?? parsed.FX_EUR_PER_RON ?? '0');
const fxUsdPerRon = Number(process.env.FX_USD_PER_RON ?? parsed.FX_USD_PER_RON ?? '0');
...
const config = {
  ...
  sentryDsn,
  fxEurPerRon: Number.isFinite(fxEurPerRon) ? fxEurPerRon : 0,
  fxUsdPerRon: Number.isFinite(fxUsdPerRon) ? fxUsdPerRon : 0
};

...
async def test_content_media_preview_signature_and_variant_error_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(content_api.media_dam, 'resolve_asset_preview_path', lambda *_args, **_kwargs: _raise(ValueError('no variant')))
    ...
    monkeypatch.setattr(content_api.media_dam, 'resolve_asset_preview_path', lambda *_args, **_kwargs: _raise(FileNotFoundError('missing file')))

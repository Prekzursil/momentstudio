  ...
  template: `
    <div class="grid gap-6">
      ...
      <app-admin-page-header
        [titleKey]="'adminUi.support.title'"
        [hintKey]="'adminUi.support.subtitle'"
      ></app-admin-page-header>
      ...
      >
        <div
          class="grid gap-3 md:grid-cols-2 xl:grid-cols-[1fr_220px_220px_1fr_220px_auto] items-end"
        >
          <app-input
          ...
          <label class="grid gap-1 text-sm text-slate-700 dark:text-slate-200">
            <span class="text-xs font-medium text-slate-600 dark:text-slate-300">{{
              'adminUi.support.filters.topic' | translate
            }}</span>
            <select
          ...
          <label class="grid gap-1 text-sm text-slate-700 dark:text-slate-200">
            <span class="text-xs font-medium text-slate-600 dark:text-slate-300">{{
              'adminUi.support.filters.status' | translate
            }}</span>
            <select
              class="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/40 [color-scheme:light] dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:[color-scheme:dark]"
              [(ngModel)]="status"
          ...
          <label class="grid gap-1 text-sm text-slate-700 dark:text-slate-200">
            <span class="text-xs font-medium text-slate-600 dark:text-slate-300">{{
              'adminUi.support.filters.assignee' | translate
            }}</span>
            <select
              class="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/40 [color-scheme:light] dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:[color-scheme:dark]"
          ...
          <app-button
            size="sm"
            [label]="'adminUi.support.filters.apply' | translate"
            (action)="applyFilters()"
          ></app-button>
        </div>
        ...
        <div class="grid lg:grid-cols-[1fr_420px] gap-4 items-start">
            ...
            >
              ...
              <div class="mt-3 grid gap-3">
                <p class="text-xs text-slate-600 dark:text-slate-300">
                  {{
                    'adminUi.support.sla.current'
                      | translate: { first: slaFirstReplyHours, resolution: slaResolutionHours }
                  }}
                </p>
                ...
                <div *ngIf="canEditSlaSettings()" class="flex items-center gap-2 text-sm">
                  ...
                  <span
                    ...
                    *ngIf="slaSettingsMessage"
                    >{{ slaSettingsMessage }}</span
                  >
                  <span class="text-xs text-rose-700 dark:text-rose-300" *ngIf="slaSettingsError">{{
                    slaSettingsError
                  }}</span>
                </div>
              </div>
                ...
                <thead class="bg-slate-50 text-slate-700 dark:bg-slate-800/70 dark:text-slate-200">
                  <tr>
                    <th class="text-left font-semibold px-3 py-2">
                      {{ 'adminUi.support.table.date' | translate }}
                    </th>
                    <th class="text-left font-semibold px-3 py-2">
                      {{ 'adminUi.support.table.topic' | translate }}
                    </th>
                    <th class="text-left font-semibold px-3 py-2">
                      {{ 'adminUi.support.table.status' | translate }}
                    </th>
                    <th class="text-left font-semibold px-3 py-2">
                      {{ 'adminUi.support.table.sla' | translate }}
                    </th>
                    <th class="text-left font-semibold px-3 py-2">
                      {{ 'adminUi.support.table.assignee' | translate }}
                    </th>
                    <th class="text-left font-semibold px-3 py-2">
                      {{ 'adminUi.support.table.from' | translate }}
                    </th>
                    <th class="text-left font-semibold px-3 py-2">
                      {{ 'adminUi.support.table.order' | translate }}
                    </th>
                  </tr>
                </thead>
            ...
            >
              ...
              <div class="flex items-center gap-2">
                <app-button
                  size="sm"
                  [disabled]="!hasPrev()"
                  [label]="'adminUi.support.prev' | translate"
                  (action)="prev()"
                ></app-button>
                <app-button
                  size="sm"
                  [disabled]="!hasNext()"
                  [label]="'adminUi.support.next' | translate"
                  (action)="next()"
                ></app-button>
              </div>
            </div>
          ...
          >
            <div class="text-sm font-semibold text-slate-900 dark:text-slate-50">
              {{ 'adminUi.support.detail.title' | translate }}
            </div>
            ...
            <div *ngIf="selected()" class="grid gap-3 text-sm text-slate-700 dark:text-slate-200">
              ...
              <div class="grid gap-1">
                ...
                >
                  {{ 'adminUi.support.detail.meta' | translate }}
                </div>
                <div class="flex flex-wrap gap-2">
                  ...
                  <span
                    *ngIf="selected()!.assignee"
                    class="inline-flex rounded-full px-2 py-0.5 text-xs border border-slate-200 dark:border-slate-700"
                  >
                    {{ 'adminUi.support.detail.assigneeChip' | translate }}:
                    {{ formatAgent(selected()!.assignee!) }}
                  </span>
              ...
              <div class="grid gap-1">
                <div
                  class="text-xs font-semibold tracking-wide uppercase text-slate-500 dark:text-slate-400"
                >
                  {{ 'adminUi.support.detail.message' | translate }}
                </div>
                <div
                  ...
                  >
                    <div class="flex items-center justify-between gap-3">
                      ...
                      >
                        {{
                          m.from_admin
                            ? ('adminUi.support.detail.fromSupport' | translate)
                            : ('adminUi.support.detail.fromCustomer' | translate)
                        }}
                      </div>
              ...
              <div class="grid gap-2">
                ...
                >
                  <div class="flex items-center justify-between gap-2">
                    <div
                      class="text-xs font-semibold tracking-wide uppercase text-slate-500 dark:text-slate-300"
                    >
                  ...
                  <div
                    *ngIf="!cannedLoading() && !cannedResponses().length"
                    class="text-sm text-slate-600 dark:text-slate-300"
                  >
                    ...
                    >
                      <div class="grid gap-0.5">
                        ...
                        <div class="text-xs text-slate-500 dark:text-slate-300">
                          {{
                            (t.is_active
                              ? 'adminUi.support.templates.active'
                              : 'adminUi.support.templates.inactive'
                            ) | translate
                          }}
                        </div>
                      </div>
                        ...
                        <app-button
                          ...
                          [label]="
                            t.is_active
                              ...
                              : ('adminUi.actions.restore' | translate)
                          "
                  ...
                  >
                    <div class="text-sm font-semibold text-slate-900 dark:text-slate-50">
                      {{
                        (templateEditingId
                          ? 'adminUi.support.templates.editTitle'
                          : 'adminUi.support.templates.newTitle'
                        ) | translate
                      }}
                    </div>
                    ...
                    <app-input
                      ...
                      [(value)]="templateTitle"
                      [placeholder]="
                        'adminUi.support.templates.fields.titlePlaceholder' | translate
                      "
                    ...
                    >
                      ...
                      <textarea
                        ...
                        maxlength="10000"
                        [placeholder]="
                          'adminUi.support.templates.fields.bodyPlaceholder' | translate
                        "
                      ></textarea>
                    </label>
              ...
              <div class="grid gap-2">
                ...
                <div class="flex justify-end">
                  <app-button
                    size="sm"
                    [disabled]="saving()"
                    [label]="'adminUi.support.detail.save' | translate"
                    (action)="save()"
                  ></app-button>
                </div>
              </div>
...
export class AdminSupportComponent implements OnInit {
  ...
  saveSlaSettings(): void {
    ...
    this.slaSettingsMessage = null;
    this.api
      .updateSlaSettings({ first_reply_hours: first, resolution_hours: resolution })
      .subscribe({
        next: (updated) => {
          ...
          this.slaResolutionHours = Number.isFinite(nextResolution)
            ? Math.trunc(nextResolution)
            : resolution;
          this.slaFirstReplyHoursDraft = this.slaFirstReplyHours;
          this.slaResolutionHoursDraft = this.slaResolutionHours;
  ...
  slaInfo(row: AdminContactSubmissionListItem): { label: string; class: string } | null {
    ...
    const prefix = this.translate.instant(
      isReply ? 'adminUi.support.sla.reply' : 'adminUi.support.sla.resolve',
    );
    const suffix =
      delta < 0
  ...
  saveTemplate(): void {
    ...
    const fail = (err: any) => {
      this.templateSaving.set(false);
      const msg =
        err?.error?.detail || this.translate.instant('adminUi.support.templates.errors.save');
    ...
    this.api.updateCannedResponse(t.id, { is_active: !t.is_active }).subscribe({
      next: (updated) => {
        this.cannedResponses.set(
          this.cannedResponses().map((row) => (row.id === updated.id ? updated : row)),
        );
      },
      error: () =>
        this.toast.error(this.translate.instant('adminUi.support.templates.errors.save')),
    });
  }
  ...
  deleteTemplate(t: SupportCannedResponseRead): void {
    ...
    this.api.deleteCannedResponse(t.id).subscribe({
      ...
      error: () =>
        this.toast.error(this.translate.instant('adminUi.support.templates.errors.delete')),
    });
  }
  ...
  sendReply(): void {
    ...
    this.api.addMessage(selected.id, message).subscribe({
      next: (updated) => {
        ...
        this.replyMessage = '';
        this.items.set(
          this.items().map((it) => (it.id === updated.id ? { ...it, status: updated.status } : it)),
        );

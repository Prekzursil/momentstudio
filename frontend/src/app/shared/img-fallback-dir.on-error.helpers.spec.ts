...
/** Golden WU img-fallback-dir-on-error -- onError. */
describe('ImgFallbackDirective onError (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(ImgFallbackDirective.prototype) as ImgFallbackDirective;
    ...
    expect(() => { (cmp as any).onError(); }).not.toThrow();
  });
});

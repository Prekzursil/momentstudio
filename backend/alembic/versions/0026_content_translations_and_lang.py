...
def upgrade() -> None:
    op.add_column(
        "content_blocks", sa.Column("lang", sa.String(length=10), nullable=True)
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "content_block_id", "lang", name="uq_content_block_translations_block_lang"
        ),
    )
    op.create_index(
        "ix_content_block_translations_lang", "content_block_translations", ["lang"]
    )


def downgrade() -> None:
    op.drop_index(
        "ix_content_block_translations_lang", table_name="content_block_translations"
    )

    ...
    >
      <div class="flex items-center justify-between">
        ...
        <app-button
          size="sm"
          variant="ghost"
          [label]="'account.addresses.add' | translate"
          (action)="account.openAddressForm()"
        ></app-button>
      </div>
      ...
      <div
        *ngIf="account.showAddressForm"
        class="rounded-lg border border-slate-200 p-3 dark:border-slate-700"
      >
        <app-address-form
          [model]="account.addressModel"
          (save)="account.saveAddress($event)"
          (cancel)="account.closeAddressForm()"
        ></app-address-form>
      </div>
      <div
        *ngIf="
          account.addressesLoaded() &&
          !account.addressesLoading() &&
          !account.addressesError() &&
          account.addresses().length === 0 &&
          !account.showAddressForm
        "
      ...
      >
        <div class="flex items-center justify-between">
          <span class="font-semibold text-slate-900 dark:text-slate-50">
            <ng-container [ngSwitch]="addr.label">
              <ng-container *ngSwitchCase="'home'">{{
                'account.addresses.labels.home' | translate
              }}</ng-container>
              <ng-container *ngSwitchCase="'work'">{{
                'account.addresses.labels.work' | translate
              }}</ng-container>
              <ng-container *ngSwitchCase="'other'">{{
                'account.addresses.labels.other' | translate
              }}</ng-container>
              <ng-container *ngSwitchDefault>{{
                addr.label || ('account.addresses.labels.address' | translate)
              }}</ng-container>
            </ng-container>
          </span>
          <div class="flex items-center gap-2 text-xs">
            <span
              *ngIf="addr.is_default_shipping"
              ...
              >{{ 'account.addresses.defaultShipping' | translate }}</span
            >
            <span
              *ngIf="addr.is_default_billing"
              class="rounded-full bg-slate-100 px-2 py-0.5 dark:bg-slate-800"
              >{{ 'account.addresses.defaultBilling' | translate }}</span
            >
          </div>
          <div class="flex flex-wrap gap-2">
            ...
            <app-button
              size="sm"
              variant="ghost"
              [label]="'account.addresses.edit' | translate"
              (action)="account.editAddress(addr)"
            ></app-button>
            <app-button
              size="sm"
              variant="ghost"
            ...
            <app-button
              size="sm"
              variant="ghost"
              [label]="'account.addresses.delete' | translate"
              (action)="account.removeAddress(addr.id)"
            ></app-button>
          </div>
        </div>
        <span
        ...
        >
        <span
          >{{ addr.city }}<ng-container *ngIf="addr.region">, {{ addr.region }}</ng-container

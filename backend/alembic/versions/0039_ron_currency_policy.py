...
def upgrade() -> None:
    ...
    promo_codes = sa.table(
        ...
        sa.column("amount_off", sa.Numeric()),
    )
    ...
    def count_non_ron(table: str) -> int:
        ...
        condition = sa.and_(
            table_ref.c.currency.is_not(None), table_ref.c.currency != "RON"
        )
    ...
    op.execute(
        "UPDATE products SET currency='RON' WHERE currency IS NOT NULL AND UPPER(currency)='RON'"
    )
    op.execute(
        "UPDATE orders SET currency='RON' WHERE currency IS NOT NULL AND UPPER(currency)='RON'"
    )
    op.execute(
        "UPDATE promo_codes SET currency='RON' WHERE currency IS NOT NULL AND UPPER(currency)='RON'"
    )

    offenders = {
        table: count_non_ron(table) for table in ("products", "orders", "promo_codes")
    }
    offenders = {table: count for table, count in offenders.items() if count > 0}
    if offenders:
        detail = ", ".join(
            f"{table}={count}" for table, count in sorted(offenders.items())
        )
        print(
        ...
        op.execute(
            "UPDATE products SET currency='RON' WHERE currency IS NOT NULL AND currency <> 'RON'"
        )
        op.execute(
            "UPDATE orders SET currency='RON' WHERE currency IS NOT NULL AND currency <> 'RON'"
        )
        op.execute(
    ...
    op.execute(
        "UPDATE promo_codes SET currency='RON' WHERE amount_off IS NOT NULL AND currency IS NULL"
    )

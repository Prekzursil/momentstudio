...
let packageVersion = '';
try {
  const packageJsonPath = assertPathWithinRoot(
    frontendRoot,
...
const parsed = envPath
  ? parseDotEnv(fs.readFileSync(assertPathWithinRoot(frontendRoot, envPath, 'env file'), 'utf8'))
  : {};
...
const outPath = assertPathWithinRoot(
  frontendRoot,
  path.join(frontendRoot, 'src', 'assets', 'app-config.js'),
  'output file',
);

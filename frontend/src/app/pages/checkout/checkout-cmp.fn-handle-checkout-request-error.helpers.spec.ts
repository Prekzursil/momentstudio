...
/** Golden WU checkout-cmp-fn-handle-checkout-request-error -- handleCheckoutRequestError. */
describe('CheckoutComponent handleCheckoutRequestError (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).handleCheckoutRequestError(undefined as any); }).not.toThrow();
  });
});

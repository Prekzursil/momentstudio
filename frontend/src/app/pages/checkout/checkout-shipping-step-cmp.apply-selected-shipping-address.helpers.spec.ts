...
/** Golden WU checkout-shipping-step-cmp-apply-selected-shipping-address -- applySelectedShippingAddress. */
describe('CheckoutShippingStepComponent applySelectedShippingAddress (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applySelectedShippingAddress(); }).not.toThrow();
  });
});

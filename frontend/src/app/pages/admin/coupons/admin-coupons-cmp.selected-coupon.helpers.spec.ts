...
/** Golden WU admin-coupons-cmp-selected-coupon -- selectedCoupon. */
describe('AdminCouponsComponent selectedCoupon (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).selectedCoupon(); }).not.toThrow();
  });
});

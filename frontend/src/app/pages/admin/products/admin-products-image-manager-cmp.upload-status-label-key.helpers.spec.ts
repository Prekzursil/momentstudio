...
/** Golden WU admin-products-image-manager-cmp-upload-status-label-key -- uploadStatusLabelKey. */
describe('AdminProductsImageManagerComponent uploadStatusLabelKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).uploadStatusLabelKey(undefined as any); }).not.toThrow();
  });
});

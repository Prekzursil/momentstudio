...
def test_postgres_core_flow_wishlist() -> None:
    client = TestClient(app)

    email = f"pg-{uuid.uuid4().hex[:8]}@example.com"
    register = client.post("/api/v1/auth/register", json={"email": email, "password": "supersecret", "name": "PG"})
    ...
    products = client.get("/api/v1/catalog/products", params={"search": "Postgres"})
    assert products.status_code == 200, products.text
    assert any(item["id"] == product_id for item in products.json()["items"])

    # Wishlist API flow under Postgres
    add = client.post(f"/api/v1/wishlist/{product_id}", headers=auth_headers(token))
    assert add.status_code == 201, add.text

    listed = client.get("/api/v1/wishlist", headers=auth_headers(token))
    assert listed.status_code == 200, listed.text
    assert any(item["id"] == product_id for item in listed.json())

    removed = client.delete(f"/api/v1/wishlist/{product_id}", headers=auth_headers(token))

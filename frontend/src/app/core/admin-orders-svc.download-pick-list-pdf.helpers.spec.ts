...
/** Golden WU admin-orders-svc-download-pick-list-pdf -- downloadPickListPdf. */
describe('AdminOrdersService downloadPickListPdf (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadPickListPdf(''); }).not.toThrow();
  });
});

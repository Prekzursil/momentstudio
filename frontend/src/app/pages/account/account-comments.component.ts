    ...
    >
      <div class="flex items-center justify-between gap-3">
        <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-50">
          {{ 'account.sections.comments' | translate }}
        </h2>
        <app-button
          size="sm"
          variant="ghost"
          [label]="'account.comments.actions.refresh' | translate"
          (action)="account.loadMyComments(account.myCommentsPage)"
        ></app-button>
      </div>
      ...
      <ng-template #myCommentsBody>
        ...
        >
          ...
          <a routerLink="/blog" class="text-indigo-600 dark:text-indigo-300 font-medium">{{
            'account.comments.empty.browse' | translate
          }}</a
          >.
        </div>
        ...
        <div *ngIf="!account.myCommentsError() && account.myComments().length" class="grid gap-3">
          <div
            *ngFor="let c of account.myComments()"
            class="rounded-lg border border-slate-200 p-3 grid gap-2 dark:border-slate-700"
          >
            <div class="flex items-start justify-between gap-3">
              <a
                [routerLink]="['/blog', c.post_slug]"
                class="font-semibold text-slate-900 dark:text-slate-50 hover:underline"
              >
                {{ c.post_title || c.post_slug }}
              </a>
              <span
                class="text-xs rounded-full px-2 py-1 whitespace-nowrap"
                [ngClass]="account.commentStatusChipClass(c.status)"
              >
                {{ 'account.comments.status.' + c.status | translate }}
              </span>
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-400">
              {{ account.formatTimestamp(c.created_at) }}
            </p>
            <p *ngIf="c.body" class="text-sm text-slate-700 dark:text-slate-200">{{ c.body }}</p>
            <p
              *ngIf="!c.body && c.status === 'deleted'"
              class="text-sm text-slate-500 dark:text-slate-400"
            >
              {{ 'account.comments.messages.deleted' | translate }}
            </p>
            <p
              *ngIf="!c.body && c.status === 'hidden'"
              class="text-sm text-slate-500 dark:text-slate-400"
            >
              {{ 'account.comments.messages.hidden' | translate }}
            </p>
            ...
            >
              <p class="text-xs text-slate-500 dark:text-slate-400">
                {{
                  'account.comments.replyingTo'
                    | translate
                      : { author: c.parent.author_name || ('blog.comments.anonymous' | translate) }
                }}
              </p>
            ...
            <div *ngIf="c.reply_count" class="text-sm text-slate-700 dark:text-slate-200">
              ...
              {{
                c.reply_count === 1
                  ? ('account.comments.replies.one' | translate)
                  : ('account.comments.replies.many' | translate)
              }}
                ...
                {{
                  'account.comments.latest'
                    | translate
                      : {
                          author:
                            c.last_reply.author_name || ('blog.comments.anonymous' | translate),
                          snippet: c.last_reply.snippet,
                        }
                }}
              </span>
            </div>
          ...
          >
            <span>
              {{
                'account.comments.pageLabel'
                  | translate
                    : {
                        page: account.myCommentsMeta()?.page || 1,
                        total: account.myCommentsMeta()?.total_pages || 1,
                      }
              }}
            </span>
            <div class="flex gap-2">
              ...
              <app-button
                ...
                [disabled]="
                  (account.myCommentsMeta()?.page || 1) >=
                  (account.myCommentsMeta()?.total_pages || 1)
                "

...
/** Golden WU account-state-profile-has-unsaved-changes -- profileHasUnsavedChanges. */
describe('AccountState profileHasUnsavedChanges (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).profileHasUnsavedChanges(); }).not.toThrow();
  });
});

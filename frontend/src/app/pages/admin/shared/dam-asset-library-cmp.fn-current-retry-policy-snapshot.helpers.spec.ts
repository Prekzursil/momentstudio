...
/** Golden WU dam-asset-library-cmp-fn-current-retry-policy-snapshot -- currentRetryPolicySnapshot. */
describe('DamAssetLibraryComponent currentRetryPolicySnapshot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).currentRetryPolicySnapshot(undefined as any); }).not.toThrow();
  });
});

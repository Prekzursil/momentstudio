    ...
    >
      ...
      <ng-container *ngIf="vm.auth.isAuthenticated(); else guestCoupons">
        ...
        <div class="grid gap-2 pt-2">
          <div
            class="grid gap-2 rounded-xl border border-slate-200 bg-slate-50/60 p-3 text-xs text-slate-700 dark:border-slate-800 dark:bg-slate-950/30 dark:text-slate-200"
          >
            <p class="font-semibold text-slate-800 dark:text-slate-100">
              {{ 'checkout.couponRulesTitle' | translate }}
            </p>
            ...
            <label class="flex items-center gap-2 pt-1">
              <input
                type="checkbox"
                [checked]="vm.autoApplyBestCoupon"
                (change)="vm.setAutoApplyBestCouponPreference($any($event.target).checked)"
              />
          ...
          <ng-container
            *ngIf="
              !vm.couponEligibilityLoading && !vm.couponEligibilityError && vm.couponEligibility
            "
          >
                ...
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <p class="text-sm font-medium text-slate-900 dark:text-slate-50">
                      {{
                        vm.suggestedCouponOffer.coupon.promotion?.name ||
                          vm.suggestedCouponOffer.coupon.code
                      }}
                    </p>
                    ...
                    <div
                      *ngIf="vm.suggestedCouponOffer.coupon.promotion as promo"
                      class="pt-1 flex flex-wrap gap-1"
                    >
                      <span
                      ...
                      >
                        {{
                          (promo.allow_on_sale_items
                            ...
                            : 'checkout.couponExcludesSaleItems'
                          ) | translate
                        }}
                      </span>
                    </div>
              ...
              <div class="grid gap-2">
                ...
                >
                  ...
                  <app-button
                    ...
                    variant="ghost"
                    [label]="'checkout.apply' | translate"
                    (action)="vm.applyCouponOffer(offer)"
                  ></app-button>
                </div>
              </div>
            </div>
            ...
            <details *ngIf="vm.couponEligibility.ineligible.length" class="grid gap-2">
              <summary
                class="cursor-pointer text-xs font-semibold text-slate-700 dark:text-slate-200"
              >
              ...
              <div class="grid gap-2">
                ...
                >
                  <div class="min-w-0">
                    ...
                    <p
                      *ngIf="offer.reasons?.length"
                      class="text-xs text-amber-700 dark:text-amber-300 pt-1"
                    >

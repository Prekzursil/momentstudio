...
/** Golden WU admin-ops-cmp-sameday-sync-running -- samedaySyncRunning. */
describe('AdminOpsComponent samedaySyncRunning (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).samedaySyncRunning(); }).not.toThrow();
  });
});

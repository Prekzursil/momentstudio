...
/** Golden WU admin-orders-svc-download-pick-list-csv -- downloadPickListCsv. */
describe('AdminOrdersService downloadPickListCsv (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadPickListCsv(''); }).not.toThrow();
  });
});

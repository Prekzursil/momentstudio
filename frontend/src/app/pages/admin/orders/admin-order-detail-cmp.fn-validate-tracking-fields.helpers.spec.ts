...
/** Golden WU admin-order-detail-cmp-fn-validate-tracking-fields -- validateTrackingFields. */
describe('AdminOrderDetailComponent validateTrackingFields (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).validateTrackingFields(undefined as any, '', ''); }).not.toThrow();
  });
});

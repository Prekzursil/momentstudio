      ...
      <ng-template #securityBody>
        <div class="grid gap-1">
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400">
            {{ 'account.security.subtitle' | translate }}
          </p>
        </div>

        <div class="grid gap-3">
          <div
            class="rounded-xl border border-slate-200 p-3 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2"
          >
            <div class="grid gap-1">
              <p class="font-semibold text-slate-900 dark:text-slate-50">
                {{ 'account.security.password.title' | translate }}
              </p>
              <p class="text-xs text-slate-500 dark:text-slate-400">
                {{ 'account.security.password.copy' | translate }}
              </p>
            </div>
            <app-button
          ...
          <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-800 grid gap-3">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div class="grid gap-1">
                <p class="font-semibold text-slate-900 dark:text-slate-50">
                  {{ 'account.security.twoFactor.title' | translate }}
                </p>
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  {{ 'account.security.twoFactor.copy' | translate }}
                </p>
              </div>
              <span
              ...
              >
                {{
                  (account.twoFactorStatus()?.enabled
                    ? 'account.security.twoFactor.enabled'
                    : 'account.security.twoFactor.disabled'
                  ) | translate
                }}
              </span>
            </div>
            ...
            <div
              *ngIf="account.twoFactorLoading()"
              class="text-sm text-slate-600 dark:text-slate-300"
            >
              {{ 'notifications.loading' | translate }}
            </div>

            <p *ngIf="account.twoFactorError()" class="text-xs text-rose-700 dark:text-rose-300">
              {{ account.twoFactorError() }}
            </p>
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  <p class="text-sm font-semibold text-slate-900 dark:text-slate-50">
                    {{ 'account.security.twoFactor.recoveryTitle' | translate }}
                  </p>
                  <app-button
                    size="sm"
                    variant="ghost"
                ...
                <p class="text-xs text-slate-600 dark:text-slate-300">
                  {{ 'account.security.twoFactor.recoveryCopy' | translate }}
                </p>
                ...
                <label class="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">
                  ...
                  <div class="relative">
                    ...
                    <button
                      ...
                      [attr.aria-label]="
                        (showTwoFactorManagePassword ? 'auth.hidePassword' : 'auth.showPassword')
                          | translate
                      "
                    >
            ...
            <ng-template #twoFactorSetup>
              <ng-container
                *ngIf="
                  account.twoFactorSetupSecret && account.twoFactorSetupUrl;
                  else twoFactorStart
                "
              >
                <p class="text-sm text-slate-700 dark:text-slate-200">
                  {{ 'account.security.twoFactor.setupHint' | translate }}
                </p>

                <div class="grid gap-4 sm:grid-cols-[auto_1fr] sm:items-start">
                  <div
                    class="rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900 grid gap-2 justify-items-center"
                  >
                    <p class="text-xs text-slate-500 dark:text-slate-400 text-center">
                      {{ 'account.security.twoFactor.qrHint' | translate }}
                    </p>
                  ...
                  <label class="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">
                    ...
                    <div class="relative">
                      ...
                      <button
                        ...
                        [attr.aria-label]="
                          (showTwoFactorSetupPassword ? 'auth.hidePassword' : 'auth.showPassword')
                            | translate
                        "
                      >
          ...
          <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-800 grid gap-3">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div class="grid gap-1">
                <p class="font-semibold text-slate-900 dark:text-slate-50">
                  {{ 'account.security.passkeys.title' | translate }}
                </p>
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  {{ 'account.security.passkeys.copy' | translate }}
                </p>
              </div>
              <span
                class="rounded-full px-2 py-0.5 text-[11px] font-semibold"
              ...
              >
                {{
                  (account.passkeys().length > 0
                    ? 'account.security.passkeys.enabled'
                    : 'account.security.passkeys.disabled'
                  ) | translate
                }}
              </span>
            </div>
            ...
            <p
              *ngIf="!account.passkeysSupported()"
            ...
            <div
              *ngIf="account.passkeysSupported() && account.passkeysLoading()"
              class="text-sm text-slate-600 dark:text-slate-300"
            >
              {{ 'notifications.loading' | translate }}
            </div>

            <p *ngIf="account.passkeysError()" class="text-xs text-rose-700 dark:text-rose-300">
              {{ account.passkeysError() }}
            </p>

            <p
              *ngIf="
                account.passkeysSupported() &&
                !account.passkeysLoading() &&
                account.passkeys().length === 0
              "
              class="text-sm text-slate-700 dark:text-slate-200"
            >
            ...
            >
              ...
              >
                <div class="min-w-0 grid gap-1">
                  ...
                  <p class="text-xs text-slate-500 dark:text-slate-400">
                    {{ 'account.security.passkeys.created' | translate }}:
                    {{ p.created_at | date: 'medium' }}
                    <span *ngIf="p.last_used_at">
                      • {{ 'account.security.passkeys.lastUsed' | translate }}:
                      {{ p.last_used_at | date: 'medium' }}</span
                    >
                    <span *ngIf="p.device_type"> • {{ p.device_type }}</span>
                    <span *ngIf="p.backed_up">
                      • {{ 'account.security.passkeys.backedUp' | translate }}</span
                    >
                  </p>
                </div>
                <ng-container
                  *ngIf="account.removePasskeyConfirmId !== p.id; else passkeyRemoveConfirm"
                >
                  <app-button
                    size="sm"
                    variant="ghost"
                ...
                <ng-template #passkeyRemoveConfirm>
                  <div class="w-full grid gap-2 sm:grid-cols-[2fr_auto_auto] sm:items-end">
                    ...
                    >
                      ...
                      <div class="relative">
                        ...
                        <button
                          ...
                          [attr.aria-label]="
                            (showRemovePasskeyPassword ? 'auth.hidePassword' : 'auth.showPassword')
                              | translate
                          "
                        >
                    ...
                    <app-button
                      ...
                      [disabled]="
                        account.removingPasskeyId === p.id || !account.removePasskeyPassword.trim()
                      "
              ...
              <label class="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">
                ...
                <div class="relative">
                  ...
                  <button
                    ...
                    [attr.aria-label]="
                      (showPasskeyPassword ? 'auth.hidePassword' : 'auth.showPassword') | translate
                    "
                  >
          ...
          <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-800 grid gap-3">
            <div class="grid gap-1">
              <p class="font-semibold text-slate-900 dark:text-slate-50">
                {{ 'account.security.emails.title' | translate }}
              </p>
              <p class="text-xs text-slate-500 dark:text-slate-400">
                {{ 'account.security.emails.copy' | translate }}
              </p>
              <p class="text-xs text-slate-500 dark:text-slate-400">
              ...
              <p *ngIf="account.googleEmail()" class="text-xs text-amber-800 dark:text-amber-200">
                ...
                <a href="#google-settings" class="underline underline-offset-2 ml-1">{{
                  'account.security.emails.googleWarningLink' | translate
                }}</a>
              </p>
              <p
                *ngIf="account.emailCooldownSeconds() > 0"
                class="text-xs text-amber-800 dark:text-amber-200"
              >
                {{
                  'account.cooldowns.email'
                    | translate: { time: account.formatCooldown(account.emailCooldownSeconds()) }
                }}
              </p>
            </div>
            ...
            <p
              *ngIf="account.secondaryEmailsError()"
              class="text-xs text-rose-700 dark:text-rose-300"
            >
              {{ account.secondaryEmailsError() }}
            </p>
            <p
              *ngIf="account.secondaryEmailMessage"
              class="text-xs text-slate-600 dark:text-slate-300"
            >
              {{ account.secondaryEmailMessage }}
            </p>

            <div
              *ngIf="account.secondaryEmailsLoading()"
              class="text-sm text-slate-600 dark:text-slate-300"
            >
              {{ 'notifications.loading' | translate }}
            </div>
                  ...
                  <p class="text-xs text-slate-500 dark:text-slate-400">
                    ...
                    {{
                      account.emailVerified()
                        ? ('account.security.status.verified' | translate)
                        : ('account.security.status.unverified' | translate)
                    }}
                  </p>
                </div>
              </li>
              ...
              >
                    ...
                    *ngIf="!e.verified"
                    ...
                    [label]="
                      account.secondaryEmailResendRemainingSeconds(e.id) > 0
                        ? ('account.security.emails.resendIn'
                          | translate
                            : { seconds: account.secondaryEmailResendRemainingSeconds(e.id) })
                ...
                >
                  <label class="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">
                    ...
                    <div class="relative">
                      ...
                      <button
                        ...
                        class="absolute inset-y-0 right-2 inline-flex items-center text-xs font-semibold text-slate-600 dark:text-slate-300"
                        (click)="
                          showRemoveSecondaryEmailPassword = !showRemoveSecondaryEmailPassword
                        "
                        [attr.aria-label]="
                          (showRemoveSecondaryEmailPassword
                            ? 'auth.hidePassword'
                            : 'auth.showPassword'
                          ) | translate
                        "
                  ...
                  <app-button
                    ...
                    [label]="'account.security.actions.confirm' | translate"
                    [disabled]="
                      account.removingSecondaryEmail || !account.removeSecondaryEmailPassword.trim()
                    "
                ...
                >
                  ...
                  <app-button
                    ...
                    [label]="'account.security.actions.confirm' | translate"
                    [disabled]="
                      account.verifyingSecondaryEmail || !account.secondaryVerificationToken.trim()
                    "
                  ></app-button>
                  <app-button
                    size="sm"
                ...
                <p
                  *ngIf="
                    !e.verified &&
                    account.secondaryVerificationEmailId === e.id &&
                  ...
                  <label class="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">
                    ...
                    <div class="relative">
                      ...
                      <button
                        ...
                        [attr.aria-label]="
                          (showMakePrimaryPassword ? 'auth.hidePassword' : 'auth.showPassword')
                            | translate
                        "
                      >
          ...
          <div
            id="google-settings"
            class="rounded-xl border border-slate-200 p-3 dark:border-slate-800 grid gap-2"
          >
            <div class="flex flex-col sm:flex-row sm:items-center gap-3 text-sm">
              ...
              <div class="min-w-0">
                <p class="font-semibold text-slate-900 dark:text-slate-50">
                  {{ 'account.security.google.title' | translate }}
                </p>
                <p class="text-slate-600 dark:text-slate-300 truncate">
                  {{ account.googleEmail() || ('account.security.google.none' | translate) }}
                </p>
              </div>
              <div class="flex flex-col sm:flex-row gap-2 sm:ml-auto w-full sm:w-auto">
                <div class="relative w-full sm:w-auto">
                  ...
                  <button
                    ...
                    [attr.aria-label]="
                      (showGooglePassword ? 'auth.hidePassword' : 'auth.showPassword') | translate
                    "
                  >
                ...
                <app-button
                  ...
                  *ngIf="!account.googleEmail()"
                  [disabled]="
                    account.googleBusy || (account.googleLinkPending && !account.googlePassword)
                  "
            ...
            <p *ngIf="account.googleError" class="text-xs text-rose-700 dark:text-rose-300">
              {{ account.googleError }}
            </p>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">
              {{ 'account.security.google.copy' | translate }}
            </p>
          </div>
          ...
          <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-800 grid gap-3">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div class="grid gap-1">
                <h3 class="text-base font-semibold text-slate-900 dark:text-slate-50">
                  {{ 'account.security.devices.title' | translate }}
                </h3>
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  {{ 'account.security.devices.copy' | translate }}
                </p>
              </div>
              <app-button
                *ngIf="!account.revokeOtherSessionsConfirming"
                ...
                [disabled]="
                  account.sessionsLoading() ||
                  account.revokingOtherSessions ||
                  account.otherSessionsCount() === 0
                "
            ...
            >
              <label class="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">
                ...
                <div class="relative">
                  ...
                  <button
                    ...
                    [attr.aria-label]="
                      (showRevokeOtherSessionsPassword ? 'auth.hidePassword' : 'auth.showPassword')
                        | translate
                    "
                  >
              ...
              <app-button
                ...
                [label]="'account.security.actions.confirm' | translate"
                [disabled]="
                  account.revokingOtherSessions || !account.revokeOtherSessionsPassword.trim()
                "
            ...
            <div
              *ngIf="account.sessionsLoading()"
              class="text-sm text-slate-600 dark:text-slate-300"
            >
              {{ 'notifications.loading' | translate }}
            </div>

            <p *ngIf="account.sessionsError()" class="text-xs text-rose-700 dark:text-rose-300">
              {{ account.sessionsError() }}
            </p>

            <p
              *ngIf="!account.sessionsLoading() && account.sessions().length === 0"
              class="text-sm text-slate-700 dark:text-slate-200"
            >
            ...
            <ul *ngIf="!account.sessionsLoading() && account.sessions().length" class="grid gap-2">
              ...
              >
                <div class="flex flex-wrap items-center gap-2">
                  <p
                    class="text-sm font-medium text-slate-900 dark:text-slate-50 truncate max-w-full"
                  >
                    {{
                      s.user_agent || ('account.security.devices.unknownDevice' | translate)
                        | slice: 0 : 90
                    }}
                  </p>
                  <span
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  {{ 'account.security.devices.created' | translate }}:
                  {{ s.created_at | date: 'medium' }} •
                  {{ 'account.security.devices.expires' | translate }}:
                  {{ s.expires_at | date: 'medium' }}
                  <span *ngIf="s.ip_address"> • IP {{ s.ip_address }}</span>
                  <span *ngIf="s.persistent">
                    • {{ 'account.security.devices.persistent' | translate }}</span
                  >
                </p>
              </li>
            </ul>
          ...
          <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-800 grid gap-3">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div class="grid gap-1">
                <h3 class="text-base font-semibold text-slate-900 dark:text-slate-50">
                  {{ 'account.security.activity.title' | translate }}
                </h3>
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  {{ 'account.security.activity.copy' | translate }}
                </p>
              </div>
              <app-button
                size="sm"
            ...
            <div
              *ngIf="account.securityEventsLoading()"
              class="text-sm text-slate-600 dark:text-slate-300"
            >
              {{ 'notifications.loading' | translate }}
            </div>
            ...
            <p
              *ngIf="account.securityEventsError()"
              class="text-xs text-rose-700 dark:text-rose-300"
            >
              {{ account.securityEventsError() }}
            </p>
            ...
            <ul
              *ngIf="!account.securityEventsLoading() && account.securityEvents().length"
              class="grid gap-2"
            >
              <li
              ...
              >
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  }}<span *ngIf="e.ip_address"> • IP {{ e.ip_address }}</span>
                </p>
          ...
          <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-800 grid gap-3">
            <h3 class="text-base font-semibold text-slate-900 dark:text-slate-50">
              {{ 'account.security.session.title' | translate }}
            </h3>
            <p class="text-sm text-slate-700 dark:text-slate-200">
              {{ 'account.security.session.copy' | translate }}
              <a
                class="text-indigo-600 dark:text-indigo-300 cursor-pointer"
                (click)="account.signOut()"
                >{{ 'account.security.session.logoutNow' | translate }}</a
              >.
            </p>
            <p *ngIf="account.idleWarning()" class="text-xs text-rose-700 dark:text-rose-300">
              {{ account.idleWarning() }}
            </p>

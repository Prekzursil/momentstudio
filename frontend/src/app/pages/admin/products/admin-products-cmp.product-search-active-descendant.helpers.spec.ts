...
/** Golden WU admin-products-cmp-product-search-active-descendant -- productSearchActiveDescendant. */
describe('AdminProductsComponent productSearchActiveDescendant (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).productSearchActiveDescendant(); }).not.toThrow();
  });
});

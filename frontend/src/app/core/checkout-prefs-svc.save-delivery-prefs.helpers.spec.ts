...
/** Golden WU checkout-prefs-svc-save-delivery-prefs -- saveDeliveryPrefs. */
describe('CheckoutPrefsService saveDeliveryPrefs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(CheckoutPrefsService.prototype) as CheckoutPrefsService;
    ...
    expect(() => { (cmp as any).saveDeliveryPrefs(undefined as any); }).not.toThrow();
  });
});

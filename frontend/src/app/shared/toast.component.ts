    ...
    <div class="fixed inset-x-0 top-4 z-50 flex flex-col items-center gap-3 px-4">
      <div
        *ngFor="let toast of messages"
        class="w-full max-w-md rounded-xl border border-slate-200 bg-white shadow-lg px-4 py-3 grid gap-1"
      ...
      >
        ...
        <div *ngIf="toast.description" class="text-xs text-slate-600">
          {{ toast.description }}
        </div>
      </div>
    </div>

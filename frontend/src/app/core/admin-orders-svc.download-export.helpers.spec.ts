...
/** Golden WU admin-orders-svc-download-export -- downloadExport. */
describe('AdminOrdersService downloadExport (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadExport('', false); }).not.toThrow();
  });
});

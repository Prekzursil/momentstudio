...
/** Golden WU admin-inventory-cmp-bulk-adjust-busy -- bulkAdjustBusy. */
describe('AdminInventoryComponent bulkAdjustBusy (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).bulkAdjustBusy(); }).not.toThrow();
  });
});
